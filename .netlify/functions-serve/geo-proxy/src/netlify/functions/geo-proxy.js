...
// netlify/functions/geo-proxy.ts
var geo_proxy_exports = {};
__export(geo_proxy_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(geo_proxy_exports);
...
var handler = async (event) => {
  var _a;
  const query = (_a = event.queryStringParameters) == null ? void 0 : _a.q;
  if (!query) return { statusCode: 400, body: 'Missing query' };
  try {
    const GEO_API_KEY = process.env.ITE_GEO_API_KEY;
    const url2 = `${GEO_API_URL}/cities?minPopulation=10000&namePrefix=${encodeURIComponent(query)}`;
